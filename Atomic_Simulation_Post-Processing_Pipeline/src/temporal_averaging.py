...
def perform_temporal_averaging(all_snapshots_processed_data):
    ...
    cu_fraction_2nd_shell_avg = grouped_atoms['cu_fraction_2nd_shell'].mean().to_dict() if 'cu_fraction_2nd_shell' in atom_frame.columns else {}
    zr_fraction_2nd_shell_avg = grouped_atoms['zr_fraction_2nd_shell'].mean().to_dict() if 'zr_fraction_2nd_shell' in atom_frame.columns else {}
    ...
    cu_fraction_3rd_shell_avg = grouped_atoms['cu_fraction_3rd_shell'].mean().to_dict() if 'cu_fraction_3rd_shell' in atom_frame.columns else {}
    zr_fraction_3rd_shell_avg = grouped_atoms['zr_fraction_3rd_shell'].mean().to_dict() if 'zr_fraction_3rd_shell' in atom_frame.columns else {}
    ...
    for atom_id, metadata in metadata_reference.items():
        if atom_id in g_i_r_avg_over_time and atom_id in volume_avg_over_time and atom_id in CN_avg_over_time:
            ...
            averaged_entry['cu_fraction_2nd_shell_temporal'] = cu_fraction_2nd_shell_avg.get(atom_id, np.nan)
            averaged_entry['zr_fraction_2nd_shell_temporal'] = zr_fraction_2nd_shell_avg.get(atom_id, np.nan)
            ...
            averaged_entry['cu_fraction_3rd_shell_temporal'] = cu_fraction_3rd_shell_avg.get(atom_id, np.nan)
            averaged_entry['zr_fraction_3rd_shell_temporal'] = zr_fraction_3rd_shell_avg.get(atom_id, np.nan)
